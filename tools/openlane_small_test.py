...
data_root = '.'  # root path for your dataset (set to repository/dataset root)
test_mode = False
mask_down_scale = 8
hm_down_scale = 16
mask_size = (1, 40, 100)
...
train_al_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='HueSaturationValue', hue_shift_limit=(-10, 10), sat_shift_limit=(-15, 15),
                 val_shift_limit=(-10, 10), p=1.0),
        ],
        p=0.7
    ),
    ...
    dict(type='ShiftScaleRotate', shift_limit=0.1, scale_limit=(-0.2, 0.2),
         rotate_limit=10, border_mode=0, p=0.6),
    dict(type='RandomResizedCrop', height=img_scale[1], width=img_scale[0],
         scale=(0.8, 1.2), ratio=(1.7, 2.7), p=0.6),
    dict(type='Resize', height=img_scale[1], width=img_scale[0], p=1),
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root + '/images/training/',
        data_list=data_root + '/images/list/training_list.txt',
        pipeline=train_pipeline,
        test_mode=False,
    ),
    val=dict(
        type=dataset_type,
        data_root=data_root + '/images/validation/',
